...
def playMatchesBetweenVersions(env, run_version, player1version, player2version, EPISODES, logger, turns_until_tau0, goes_first = 0):
    ...
    if player1version == -1:
        ...
        player1_NN = Densely_connected_net(config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size)
    ...
    if player2version == -1:
        ...
        player2_NN = Densely_connected_net(config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size)
    ...
    for e in range(EPISODES):
        ...
        while done == 0:
            ...
            if turn < turns_until_tau0:
                action, pi, MCTS_value, NN_value = players[state.currentPlayer]['agent'].act(state, 1)
            else:
                action, pi, MCTS_value, NN_value = players[state.currentPlayer]['agent'].act(state, 0)
            ...
            ### Do the action
            state, value, done, penalizeplayer = env.step(action) #the value of the newState from the POV of the new playerTurn i.e. -1 if the previous player played a winning move
