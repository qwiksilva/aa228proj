...
def playMatchesBetweenVersions(env, run_version, player1version, player2version, EPISODES, logger, turns_until_tau0, goes_first=0):
    ...
    if player1version == -1:
        ...
        player1_NN = Densely_connected_net(
            config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size)
    ...
    if player2version == -1:
        ...
        player2_NN = Densely_connected_net(
            config.REG_CONST, config.LEARNING_RATE, env.input_shape,   env.action_size)
...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory=None, goes_first=0):
    ...
    for e in range(EPISODES):
        ...
        while done == 0:
            ...
            state, value, done, penalizeplayer = env.step(action)
